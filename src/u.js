...
u.PATH_SEPARATOR = '__'
u.RESERVED_ENC_PREFIX = 'XXX'
u.RESERVED_ENC_SEPARATOR = '_'
...
u.packKeys = (obj) => {
    let processString = (str) => {
        ...
        components.forEach(word => {
            if (DYNAMO_RESERVED_WORDS.includes(word.toUpperCase())) {
                let newWord = u.RESERVED_ENC_PREFIX
                for (let i = 0; i < word.length; i++) {
                    let letter = word[i]
                    newWord += letter + u.RESERVED_ENC_SEPARATOR
                }
                word = newWord
            }
            replaced.push(word)
        })
        return str = replaced.join(u.PATH_SEPARATOR)
        // return (u.replace(str, '.', u.PATH_SEPARATOR))
    }
    ...
    if (obj instanceof Array) {
        let newArr = []
        obj.forEach((path) => {
            newArr.push(processString(path))
        })
        return newArr
    }
...
u.unpackKeys = (obj) => {
    let unpackString = (str) => {
        let isEncodedReserved = (word) => {return word.slice(0, 3) === u.RESERVED_ENC_PREFIX}
        let components = str.split(u.PATH_SEPARATOR)
        let replaced = []
        components.forEach(word => {
            if (isEncodedReserved(word)) {
                word = u.replace(word.slice(u.RESERVED_ENC_PREFIX.length), u.RESERVED_ENC_SEPARATOR, '')
            }
            replaced.push(word)
        })
        return replaced.join(u.PATH_SEPARATOR)
    }
