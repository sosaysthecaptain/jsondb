...
xit('DBObject_collection', async function() {
    ...
    await parentObj.createCollection(collectionPath)
    
    let message_0 = await parentObj.collection().createObject({
        data: {
    ...
    let passed2 = (message0_data === 'this is a message')
    ...
    // HOW MANY FUNCTIONS SHOULD THIS BE?
    // Batch get
    let read4 = await parentObj.collection(collectionPath).batchGet({
        limit: 4,
        start: 0,
        attributes: ['body']
    })
